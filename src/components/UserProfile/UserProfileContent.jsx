...
const UserProfileContent = ({ data }) => {
  ...
  const { posts, loading: feedLoading } = useSelector((state) => state.feed);
  ...
  useEffect(() => {
    ...
    fetchUserData();
  }, [data, BASE_URL, dispatch]);

  useEffect(() => {
    if (inView && !feedLoading) {
      dispatch(
        fetchPosts({
          posted_by: data,
          isReporter: true,
  ...
  }, [inView, feedLoading, data, dispatch]);
